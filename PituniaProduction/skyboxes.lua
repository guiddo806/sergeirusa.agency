...
local function changeSkybox(skyboxName)
    if not skyboxes[skyboxName] then
        warn("Скайбокс не найден!")
        return
    end
    ...
    newSkybox.SkyboxBk = skyboxData.SkyboxBk
    newSkybox.SkyboxDn = skyboxData.SkyboxDn
    newSkybox.SkyboxFt = skyboxData.SkyboxFt
    newSkybox.SkyboxLf = skyboxData.SkyboxLf
    newSkybox.SkyboxRt = skyboxData.SkyboxRt
    newSkybox.SkyboxUp = skyboxData.SkyboxUp

    newSkybox.Parent = lighting
    print("Скайбокс изменён на: " .. skyboxName)
end
...
local SkyboxSection = MainTab:AddLeftGroupbox("select skyboxes")

for name, _ in pairs(skyboxes) do
    SkyboxSection:AddButton(name, function()
        changeSkybox(name)
    end)
end
...
SaveManager:SetFolder("MyScript/saved")
SaveManager:BuildConfigSection(Window)
ThemeManager:ApplyToTab(MainTab)
Library:Notify("Script was loaded", 3)
